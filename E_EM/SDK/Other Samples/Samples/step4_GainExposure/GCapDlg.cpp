...
void CGCapDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_SLIDER_EXP, m_sldExp);
	DDX_Check(pDX, IDC_PROCESS_THRESHOLD, m_bThreshold);
}
...
BEGIN_MESSAGE_MAP(CGCapDlg, CDialog)
	...
	ON_WM_HSCROLL()
	ON_BN_CLICKED(IDC_PROCESS_THRESHOLD, &CGCapDlg::OnBnClickedProcessThreshold)
END_MESSAGE_MAP()
...
void CGCapDlg::OnBnClickedOpen()
{
	...
	MVGetGainRange(m_hCam,&fGainMin,&fGainMax);
	m_sldGain.SetRange(fGainMin,fGainMax);

	double fGain;
	MVGetGain(m_hCam,&fGain);
	m_sldGain.SetPos(fGain);
	
	CString str;
	str.Format("%d",(int)fGain);
	GetDlgItem(IDC_GAIN)->SetWindowText(str);

	double fExp,fExpMin,fExpMax;
	MVGetExposureTimeRange(m_hCam,&fExpMin,&fExpMax);
	m_sldExp.SetRange(fExpMin,fExpMax);
	MVGetExposureTime(m_hCam,&fExp);
	m_sldExp.SetPos(fExp);
	str.Format("%d",(int)fExp);
	GetDlgItem(IDC_EXP)->SetWindowText(str);
...
void CGCapDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	if ((CSliderCtrl*)pScrollBar == (CSliderCtrl*)GetDlgItem(IDC_SLIDER_TH)) 
	{
	...
	else if ((CSliderCtrl*)pScrollBar == (CSliderCtrl*)GetDlgItem(IDC_SLIDER_GAIN)) 
	{
		int gain = m_sldGain.GetPos();

		if( m_hCam != NULL )
		{
			MVSetGain(m_hCam,gain);
		}

		str.Format("%d",gain);
		GetDlgItem(IDC_GAIN)->SetWindowText(str);
	}
	else if ((CSliderCtrl*)pScrollBar == (CSliderCtrl*)GetDlgItem(IDC_SLIDER_EXP)) 
	{
		int nExp = m_sldExp.GetPos();

		if( m_hCam != NULL )
		{
			MVSetExposureTime(m_hCam,nExp);
		}
...
void CGCapDlg::OnBnClickedProcessThreshold()
{
	UpdateData(TRUE);
}
