...
BOOL InitRecConfByFile( CRecConf &recConf, CString srtPath )
{
	CIniFile IniFile;
	if (srtPath.IsEmpty())
	{
...
char *FccToName(DWORD fccHandler,BOOL bChinese)
{
...
void CDlgRecConf::OnBnClickedButtonChangeCodec()
{
	...
	{
		char *name = FccToName(opts.fccHandler,m_bChinese);
...
BOOL CDlgRecConf::OnInitDialog()
{
	...
	CString str;
	GetDlgItemText(IDC_CHK_AUTO_FILENAME,str);
	if( str.Find("自动",0) >= 0 )
	{
		m_bChinese = TRUE;
	}

	char *name = FccToName(m_conf.m_CompressOpt.fccHandler,m_bChinese);	
	if (name != NULL)
	{
		GetDlgItem(IDC_CODEC)->SetWindowText(name);
	}
