...
BOOL InitByFile( CSeqParam &Param, CString srtPath )
{
	CIniFile IniFile;
	if (srtPath.IsEmpty())
	{
		...
		srtPath = FILE_NAME_SEQ;
		srtPath = str_url + srtPath;
	}
	...
	Param.m_bIndex		  = IniFile.GetValueB(KEY_SEQUENCE_CONFIG, VALUE_BOOLEAN_INDEX, TRUE);
...
CSeqConfDlg::CSeqConfDlg(CString srtPath, CWnd* pParent)
: CDialog(CSeqConfDlg::IDD, pParent)
{
	InitByFile(m_Param, srtPath);
}
...
BOOL CSeqConfDlg::SaveParam( CString srtPath )
{
	...
	r |= IniFile.SetValueB(KEY_SEQUENCE_CONFIG, VALUE_BOOLEAN_INDEX,	m_Param.m_bIndex);
...
void CSeqConfDlg::OnBnClickedLoad()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		if (!InitByFile(m_Param, dlg.GetPathName()))
		{
			AfxMessageBox(IDS_ERR_READ_SEQ);
		}
		UpdateData(FALSE);
	}
}
